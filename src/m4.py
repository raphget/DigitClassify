# purpose: Keras model(Early Stopping Regularization)
# -------------------------------------------------
...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'    # Silences tensorflow warnings about GPU synchronization
...
def generateModel(X_train,y_train,X_test,y_test):
    ...
    # Trains the model: epochs=250, batch_size=100, (With early stopping)
    early_stopping = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=7)
    model.fit(X_train, y_train, validation_data = (X_test, y_test), epochs=250, batch_size=256, callbacks=[early_stopping])
    return model
...
def main():
    ...
    generateHeatMap(y_test,y_pred,"./output/hmm4.pdf")
    ...
    generateBarPlot(report, 'M4 Performance',"./output/barChartm4.pdf")
